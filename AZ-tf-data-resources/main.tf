...
resource "azurerm_service_plan" "rlmvp-svc-appplan" {
  count               = var.functions != "false" ? 1 : 0
...
resource "azurerm_function_app" "rlmvp-svc-function-appins" {
  ...
  location                   = var.az_region
  resource_group_name        = azurerm_resource_group.az_rg.name
  app_service_plan_id        = azurerm_service_plan.rlmvp-svc-appplan[count.index].id
...
#Azure SQL Server
resource "azurerm_mssql_server" "rlmvp-svc-sql-server" {
...
resource "azurerm_mssql_elasticpool" "rlmvp-svc-sql-elastic-pool" {
  ...
  resource_group_name = azurerm_resource_group.az_rg.name
  location            = var.az_region
  server_name         = azurerm_mssql_server.rlmvp-svc-sql-server[count.index].name
...
resource "azurerm_mssql_database" "rlmvp-svc-sql-db" {
  ...
  server_id       = azurerm_mssql_server.rlmvp-svc-sql-server[count.index].id
